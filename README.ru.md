...
- 🔄 **Авто‑обновления** через [Sparkle](https://sparkle-project.org).

## Установка

Скачай последний **`mw-x.y.z.dmg`** из [релизов](https://github.com/mc-c0rp/mw/releases), открой и перетащи **mw.app** в **Applications**. При первом запуске модель распознавания скачается автоматически.

> Без нотаризации (пока нет платного Apple Developer): при первом запуске — ПКМ по приложению → **Открыть**, чтобы пройти Gatekeeper.
...
Приложение не в песочнице (нужно для CGEvent/Accessibility), подписано с hardened runtime.
